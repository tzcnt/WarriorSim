...
- Forever Bloodthirst deals 35% AP plus 30/37/43/48 at levels 40/48/54/60 respectively.
...
- Keep the existing defensive simulation model. Missing avoidance, block, Revenge, Charge, current-health/healing, threat and crowd-control simulation are pre-existing defects, not requirements to expand this DPS simulator. Enrage can use existing incoming-damage events. The racial update adds static maximum health for Touch of the Grave, including Vitality's Stamina, without adding incoming health loss or healing.
